/** FILE DESCRIPTION -------------------------------------------------------
 FILENAME          : sglRectangleActiveAreaGetWidthRate.c
 DESCRIPTION       : sglRectangleActiveAreaGetWidthRate command shall return 
					the ratio for the distance between the mouse x-coordinate 
					and the x-coordinate of the low left corner of the rectangle 
					active area with the width of the rectangle active area
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  NAME: sglRectangleActiveAreaGetWidthRate
  DESCRIPTION:
    Function shall return the ratio for the distance between 
    the mouse x-coordinate and the x-coordinate of the low 
    left corner of the rectangle active area with the width 
    of the rectangle active area.
  SCOPE: global
  PARAMETERS:  
  ...
  RETURN:
   SGLfloat -> Horizontal rate between mouse position and left point 
   of rectangle for interactivity, range: [0.0F,1.0F]    
---------------------------------------------------------------------+*/
SGLfloat sglRectangleActiveAreaGetWidthRate(SGLfloat par_f_mouse_x, SGLfloat par_f_mouse_y, SGLfloat par_f_x1, SGLfloat par_f_y1, SGLfloat par_f_x2,
                                            SGLfloat par_f_y2)
{
    ...
    if ((((par_f_x2 - par_f_x1) > -SGL_MIN_F) && ((par_f_x2 - par_f_x1) < SGL_MIN_F))
        ...
        /* When there is no rotation, compute only x in pixels for performances */
        if (sgluFloatIsEqual(glob_pr_sglStatemachine->f_rotation_angle, 0.0F)) {
            /* Convert the origin position in pixel coordinate system */
            loc_f_x_1_pixel =
                (((glob_pr_sglStatemachine->f_modelview_matrix[0] * par_f_x1) + (glob_pr_sglStatemachine->f_modelview_matrix[4] * par_f_y1) +
            ...
            loc_f_x_2_pixel =
                (((glob_pr_sglStatemachine->f_modelview_matrix[0] * par_f_x2) + (glob_pr_sglStatemachine->f_modelview_matrix[4] * par_f_y2) +
                  glob_pr_sglStatemachine->f_modelview_matrix[12] + 1.0F) * glob_pr_sglStatemachine->f_viewport_width_div_2) +
                glob_pr_sglStatemachine->f_viewport_x1;

            /* Compute left and right rectangle position */
            if (loc_f_x_1_pixel < loc_f_x_2_pixel) {
                loc_f_min_x = loc_f_x_1_pixel;
                loc_f_max_x = loc_f_x_2_pixel;
            }
            else {
                loc_f_min_x = loc_f_x_2_pixel;
                loc_f_max_x = loc_f_x_1_pixel;
            }

            /* If rectangle is large enough, compute the rate between mouse position and rectangle width */
            loc_f_return = SGLfloat_div(par_f_mouse_x - loc_f_min_x, loc_f_max_x - loc_f_min_x);
        }
        else {
            ...
            sglConvertPointToPixel(par_f_x1, par_f_y1, &loc_f_x_1_pixel, &loc_f_y_1_pixel);
            sglConvertPointToPixel(par_f_x2, par_f_y1, &loc_f_x_2_pixel, &loc_f_y_2_pixel);

            /* Compute the projection of the mouse on a horizontal segment of the rectangle */
            loc_f_k =
