...
void sgluWriteWideText(SGLfloat par_f_x, SGLfloat par_f_y, const SGLushort * par_pus_string, SGLlong par_l_string_length)
{
    ...
    if ((par_pus_string == SGL_NULL) || (par_l_string_length < 0L)) {
        loc_b_error_detected = 1U;
    }
    else {
        while ((loc_ul_i < (SGLulong) par_l_string_length) && (par_pus_string[loc_ul_i] != 0) && (!loc_b_error_detected)) {
            loc_ul_i++;
        }
    }
    ...
    if (!loc_b_error_detected) {
        ...
        if (par_l_string_length > (SGLlong) SGLU_MAX_STRING_LENGTH) {
            ...
            oglxSetError(SGL_ERROR_SGLU_WRITEWIDETEXT, (SGLulong) par_l_string_length);
        }
        else {
            /* Nothing to do */
        }
