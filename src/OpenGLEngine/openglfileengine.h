...
namespace openglEngine {
	...
	public:
		...
		template <typename T>
		static T* getVerticesFromMemory(const unsigned char* data, size_t len, int srs, int demension, int& pt_size);

		template <typename T>
		static T* getStopsFromBinary(const char* path, int& pointsCount, int& stopsCount);

		template <typename T>
		static T* getStopsFromMemory(const unsigned char* data, size_t len, int& pointsCount, int& stopsCount);
		...
		template <typename T>
		static std::string getStringFromMemory(const unsigned char* data, size_t len);
